...
class MyHTable
{
    ...
    MyHTable() throws IOException
    {
        ...
        tableDescriptor.addFamily(new HColumnDescriptor("averageCounts"));
    ...
    {
        //Create a file.
        File averageCountFile = new File("averageCounts.txt");
        if (averageCountFile.exists())
            averageCountFile.delete();

        Scan scan = new Scan();
        scan.addColumn(Bytes.toBytes("averageCounts"), Bytes.toBytes("averageCounts"));
        ResultScanner rs = hTable.getScanner(scan);

        PrintWriter writer = new PrintWriter("averageCounts.txt", "UTF-8");
